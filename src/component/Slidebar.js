...
import './Slidebar.css'
import {setMainArea} from "../actions";
import {connect} from "react-redux";

const mapDispatchToProps = dispatch => {
    return {setMainArea: (state) => dispatch(setMainArea(state))}
};

const mapStateToProps = state => {
    return {mainAreaDisplayed: state.mainAreaDisplayed};
};
...
class Slide extends Component {
    ...
    handleClick(event) {
        this.props.setMainArea(event.target.alt)
    }
            ...
            <nav className="col-md-1 d-none d-md-block bg-dark sidebar">
                <div className="sidebar-sticky">
                    <ul className="nav flex-column">
                        <li className="nav-item text-center image" onClick={this.handleClick}>
                            <img alt="CHAT"
...
const Slidebar = connect(mapStateToProps, mapDispatchToProps)(Slide);
